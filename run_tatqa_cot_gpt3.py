...
def create_reader_request_processed(example: Dict[str, Any]):
    prompt = 'Read the following text and table, and then answer a question with explanation:\n'
    if example['text']:
...
prompt_8shot = """Read the following text and table, and then answer a question with explanation:
...
Explanation: The answer can be directly extracted from the text.
...
Read the following text and table, and then answer a question with explanation:
...
Explanation: The foreign income tax was 11.8 million in 2019 and 19.9 million in 2018. The change is -8.1 million.
Answer: -8.1 million
...
Explanation: Asia Pacific has total revenues of $6,490 thousand, Europe has total revenues of $36,898 thousand, and North America has total revenues of $68,024 thousand. Europe and North America are the 2 geographic regions with total revenues of more than $20,000 thousand.
Answer: 2
...
Explanation: The total accumulated amortisation in 2018 is $3,261 thousand and the total accumulated amortisation in 2019 is $5,580 thousand. The total accumulated amortisation in 2019 is greater than the total accumulated amortisation in 2018.
Answer: 2019
...
Explanation: The pre-tax losses in 2019 is $19,573 and the pre-tax losses in 2018 is $25,403. The net change in pre-tax losses is -$5,830. The percentage change is -22.95%.
Answer: -22.95%
"""
...
if __name__ == "__main__":
    ...
    if args.greedy:
        filename = f'outputs/tatqa_cot_gpt3_s{args.start}_e{args.end}_{dt_string}.jsonl'
    else:
        filename = f'outputs/tatqa_cot_gpt3_sc_{args.start}_e{args.end}_{dt_string}.jsonl'
    ...
    for example in tqdm(tatqa_dev):
        ...
        for r in codes:
            ans = r.split('Answer:')[-1].strip().replace('Explanation: ', '')
            if ans is not None:
